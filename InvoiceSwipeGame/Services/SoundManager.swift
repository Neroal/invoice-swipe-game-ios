...
final class SoundManager {
    ...
    private func tone(_ f: Float, _ dur: Double, _ vol: Float = 0.3, delay: Double = 0) {
        if let b = makeTone(frequency: f, duration: dur, volume: vol) {
            play(b, delay: delay)
        }
    }
    ...
    // 滑動：高頻短 sine，乾淨俐落
    func playSwipe()   { tone(780, 0.055, 0.14) }

    // 答對：小三度上行雙音，清脆
    func playCorrect() {
        tone(880,  0.10, 0.28)
        tone(1108, 0.09, 0.22, delay: 0.07)
    }

    // 答錯：可聽見的下行雙音（350 → 262 Hz），不刺耳
    func playWrong() {
        tone(350, 0.13, 0.32)
        tone(262, 0.17, 0.28, delay: 0.10)
    }

    // 小獎：明亮硬幣音
    func playCoin() {
        tone(1047, 0.08, 0.36)
        tone(1319, 0.07, 0.26, delay: 0.09)
    }

    // 失命：三音下行 sine，沉但不刺
    func playLifeLost() {
        for (i, f) in [392, 294, 196].enumerated() {
            tone(Float(f), 0.16, 0.36, delay: Double(i) * 0.13)
        }
    }

    // 倒數 beep：sine 波，輕重分明
    func playBeep(heavy: Bool = false) {
        tone(heavy ? 700 : 520, 0.08, heavy ? 0.44 : 0.26)
    }

    // 最後 5 秒 tick：高頻 sine，清晰不刺耳
    func playTick()   { tone(1050, 0.035, 0.20) }

    // 時間到：三音下行，乾淨收尾
    func playTimeUp() {
        for (i, f) in [440, 330, 220].enumerated() {
            tone(Float(f), 0.28, 0.46, delay: Double(i) * 0.16)
        }
    }
    ...
    func playFirstPrize() {
        for (i, f) in [523, 659, 784, 1047].enumerated() {
            tone(Float(f), 0.26, 0.36, delay: Double(i) * 0.13)
        }
    }
    ...
    func playGrandPrize() {
        for (i, f) in [523, 659, 784, 1047, 1319].enumerated() {
            tone(Float(f), 0.30, 0.38, delay: Double(i) * 0.10)
        }
    }
    ...
    func playSpecialPrize() {
        ...
        for (i, f) in mel.enumerated() {
            tone(f, 0.32, 0.40, delay: Double(i) * 0.14)
        }
    }
    ...
    func playCombo(streak: Int) {
        ...
        case 5:     // phase 2
            tone(1047, 0.09, 0.30)
            tone(1319, 0.08, 0.25, delay: 0.06)
            tone(1568, 0.07, 0.20, delay: 0.12)
        case 10:    // phase 3
            tone(1175, 0.08, 0.32)
            tone(1480, 0.08, 0.28, delay: 0.06)
            tone(1760, 0.07, 0.24, delay: 0.12)
            tone(2093, 0.06, 0.18, delay: 0.18)
        default:    // phase 4 (20+) 及後續每 10 連
            for (i, f) in ([1047, 1319, 1568, 2093, 2637] as [Float]).enumerated() {
                tone(f, 0.10, 0.34, delay: Double(i) * 0.06)
            }
        }
    }
}
